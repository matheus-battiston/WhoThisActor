...
class AdicionarAtorServiceTest {
    ...
    void deveAdicionarAtorAPartirDoRetornoDoTmdb() {
        AtorTMDBDtoPesquisaId response = getKeanuReeves();

        Ator resultado = adicionarAtorService.adicionarAtor(response);

        assertThat(resultado.getIdTmdb()).isEqualTo(6384L);
        assertThat(resultado.getNome()).isEqualTo("Keanu Reeves");
        assertThat(resultado.getNomeNormalizado()).isEqualTo("keanu reeves");
        assertThat(resultado.getImagem()).isEqualTo("/keanu.jpg");
        assertThat(resultado.getPopularity()).isEqualTo(10.0);
