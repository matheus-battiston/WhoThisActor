...
public class RequisicaoApiService {
    ...
    public PesquisaIdPorNomeDTO persquisarIdPorNome(String nomeAtor) {
        ...
        return restTemplate.exchange(url, GET, EMPTY, PesquisaIdPorNomeDTO.class).getBody();
    }

    public ListaProducoesTMDBDto pesquisarFilmesDoAtorPorId(Long id) {
        String url = URL_PESQUISA_CREDITOS_COMBINADOS_POR_ID +
                BARRA + id + PARAMETRO_CREDITOS_COMBINADOS + PARAMETRO_API_KEY + apiKey;
        RestTemplate restTemplate = new RestTemplate();
        return restTemplate.exchange(url, GET, EMPTY, ListaProducoesTMDBDto.class).getBody();
    }
}
